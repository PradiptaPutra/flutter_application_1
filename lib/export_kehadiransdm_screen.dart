...
class ExportKehadiransdmScreen extends StatefulWidget {
  ...
  @override
  _ExportKehadiransdmScreenState createState() => _ExportKehadiransdmScreenState();
}

class _ExportKehadiransdmScreenState extends State<ExportKehadiransdmScreen> {
  String catatan = '';
...
Future<void> _initializeBackgroundImage() async {
 ...
 Future<File?> _loadBackgroundImage() async {
  ...
  Future<void> _fetchDetailedData() async {
    if (widget.kegiatanId != null) {
      final dbHelper = DatabaseHelper();
      final data = await dbHelper.getEntriesByKegiatanIdAndCategoryAndUser(widget.kegiatanId!, 23, widget.userId); // Menyertakan kondisi kategori dan userId
      setState(() {
        detailedData = data;
      });
    }
  ...
  Future<void> _savePdf() async {
    ...
    try {
      ...
      String fileName = 'KehadiranSdm_${widget.puskesmas}.pdf';

      if (widget.kegiatanId != null) {
        final dbHelper = DatabaseHelper();
        final tanggalKegiatan = await dbHelper.getTanggalKegiatan(widget.kegiatanId!);
        if (tanggalKegiatan != null) {
          fileName = 'KehadiranSdm_${widget.puskesmas}_$tanggalKegiatan.pdf';
        }
      }
  ...
  pw.Widget _buildSummary() {
    return pw.Column(
      ...
      children: [
        ...
        pw.Text('Total Sebelum Bencana: ${widget.sebelum}'),
        pw.Text('Total Sesudah Bencana: ${widget.sesudah}'),
        pw.Text('Interpretasi Skoring Sebelum: ${widget.interpretasiSebelum ?? ''}'),
        pw.Text('Interpretasi Skoring Sesudah: ${widget.interpretasiSesudah ?? ''}'),
      ],
    );
  }
  ...
  pw.Widget _buildDetailedTable() {
    return pw.Table(
      ...
      children: [
        pw.TableRow(
          children: [
            _buildTableCell('Tenaga Kesehatan Puskesmas', isHeader: true),
